...
router.post("/register",function(req, res, next){
  ...
  db.query('select id from users where username=?', [username]).then(function([results, fields]){
...
router.post("/login", function(req, res, next){
  ...
  db.query('select id, username, email from users where username=?', [username])
  .then(function([results, fields]){
    if(results && results.length == 1){
      ...
      throw UserError('Failed Login: Invalid user credentials', "/login", 200);
    }
